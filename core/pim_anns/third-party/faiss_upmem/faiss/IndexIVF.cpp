...
void IndexIVF::filter(
    ...
    quantizer->search(n, x, nprobe, distances, labels, nullptr);
}
...
void IndexIVF::search(
    ...
    size_t nprobe = std::min(nlist, params ? params->nprobe : this->nprobe);
    ...
    auto sub_search_func = [this, k, nprobe, params](
        ...
        // xmh::Timer timer(" 1:level1 search");
        ...
        xmh::PerfCounter::Record("level1 search", duration.count());

        double t1 = getmillisecs();

        xmh::Timer timer2(" 2:level2 search");
...
#pragma omp parallel for if (nt > 1)
        ...
        // handle parallelization at level below (or don't run in
        // parallel at all)
        ...
        auto scan_one_list = [&](idx_t key,
            ...
            try {
                if (invlists->use_iterator) {
                    ...
                    xmh::Timer timer("  2.2:scan_codes");
                    ...
                    std::chrono::time_point<std::chrono::steady_clock> end_ =
                            std::chrono::steady_clock::now();
                    double time_scan_codes =
                            std::chrono::duration_cast<
                                    std::chrono::nanoseconds>(end_ - start_)
                                    .count();
                    double time_per_vector = time_scan_codes / list_size;

                    xmh::PerfCounter::Record(
                            "time_per_vector", time_per_vector);
                    xmh::PerfCounter::Record("list_size", list_size);
                    timer.end();
...
#pragma omp for schedule(dynamic)
            for (int64_t ij = 0; ij < n * nprobe; ij++) {
                ...
                xmh::Timer timer("  2.1:scan_one_list");
                ndis += scan_one_list(
                        keys[ij],
                        coarse_dis[ij],
                        local_dis.data(),
...
// /** It is a sad fact of software that a conceptually simple function like
// this
//  * becomes very complex when you factor in several ways of parallelizing +
//  * interrupt/error handling + collecting stats + min/max collection. The
//  * codepath that is used 95% of time is the one for parallel_mode = 0 */
...
//     if (params_in) {
//         params = dynamic_cast<const IVFSearchParameters*>(params_in);
//         FAISS_THROW_IF_NOT_MSG(params, "IndexIVF params have incorrect
//         type");
//     }
//     const size_t nprobe =
//             std::min(nlist, params ? params->nprobe : this->nprobe);
...
//         params = dynamic_cast<const IVFSearchParameters*>(params_in);
//         FAISS_THROW_IF_NOT_MSG(params, "IndexIVF params have incorrect
//         type"); quantizer_params = params->quantizer_params;
//     }
//     const size_t nprobe =
//             std::min(nlist, params ? params->nprobe : this->nprobe);
//     std::unique_ptr<idx_t[]> keys(new idx_t[nx * nprobe]);
...
//                     scanner->scan_codes_range(
//                             list_size, scodes.get(), ids.get(), radius,
//                             qres);
//                 }
//                 nlistv++;
//                 ndis += list_size;
...
//         std::vector<float> residuals(n * d);
//         quantizer->compute_residual_n(n, tv.x, residuals.data(),
//         assign.data());
