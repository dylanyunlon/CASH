...
struct QueryTables {
    ...
    void get_sim_table_(float* sim_table_, int length) {
        memcpy(sim_table_, sim_table, length * sizeof(float));
    }
...
struct KnnSearchResults {
    ...
    inline void add(idx_t j, float dis) {
        // return;
#ifdef ID_SORT
        if (std::fabs(heap_sim[0] - dis) < 1e-6 && heap_ids[0] <= ids[j]) {
...
struct IVFPQScannerT : QueryTables {
    ...
    void scan_list_with_table(
        ...
        for (size_t j = 0; j < ncode; j++) {
            ...
            if (counter == 4) {
                ...
                distance_four_codes<PQDecoder>(
                        ...
                        distance_3);
#if defined(TEST_LATENCY)
                dis_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
...
#if defined(TEST_LATENCY)
                sort_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
                                   std::chrono::high_resolution_clock::now() -
                                   start)
                                   .count();
#endif
        ...
        // xmh::PerfCounter::Record("dis_ns", dis_ns);
        // xmh::PerfCounter::Record("sort_ns", sort_ns);
        // printf("dis_ns: %lu, sort_ns: %lu\n", dis_ns, sort_ns);
 ...
 *
 * precompute_mode is how much we precompute (2 = precompute distance
 * tables, 1 = precompute pointers to distances, 0 = compute distances one
 * by one). Currently only 2 is supported
 *
...
struct IVFPQScanner : IVFPQScannerT<idx_t, METRIC_TYPE, PQDecoder>,
    ...
    void get_sim_table(float* sim_table_, int length) {
        // this->get_sim_table_(sim_table_, length);
        
        memcpy(sim_table_, this->sim_table, length * sizeof(float));
    }

    void get_dis0(float& dis0_) {
        this->get_dis0_(dis0_);
    }
