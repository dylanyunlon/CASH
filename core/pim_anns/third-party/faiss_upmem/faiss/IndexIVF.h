...
struct IndexIVF : Index, IndexIVFInterface {
    ...
    int* get_cluster_length() {
        int* cluster_length = new int[invlists->nlist];
        for (int i = 0; i < invlists->nlist; i++) {
            size_t list_size = invlists->list_size(i);
            cluster_length[i] = (int)list_size;
        }
        return cluster_length;
    }

    int get_max_length() {
        int cluster_length = INT32_MIN;
        for (int i = 0; i < invlists->nlist; i++) {
            int list_size = invlists->list_size(i);
            if (list_size > cluster_length) {
                cluster_length = (int)list_size;
            }
        }
        return cluster_length;
    }
    int get_min_length() {
        int cluster_length = INT32_MAX;
        for (int i = 0; i < invlists->nlist; i++) {
            int list_size = invlists->list_size(i);
            if (list_size < cluster_length) {
                cluster_length = (int)list_size;
    ...
    int get_shard_num(int shard_size) {
        int shard_num = 0;
        for (int i = 0; i < invlists->nlist; i++) {
            int list_size = invlists->list_size(i);
            if(list_size > shard_size) {
                shard_num += (list_size + shard_size - 1) / shard_size;
            } else {
                shard_num++;
            }
        }
    ...
    int get_max_shard_num(int shard_size) {
        int max_shard_num = 0;
        for (int i = 0; i < invlists->nlist; i++) {
            int s_num = 0;
            int list_size = invlists->list_size(i);
            if (list_size > shard_size) {
                s_num += (list_size + shard_size - 1) / shard_size;
            } else {
                s_num++;
            }
            if (s_num > max_shard_num) {
                max_shard_num = s_num;
            }
        }
    ...
    int get_length_greater_than_percent(double percent) {
        std::vector<int> lengths;
        
        for (int i = 0; i < invlists->nlist; i++) {
            int list_size = invlists->list_size(i);
            lengths.push_back(list_size);
        }

        std::sort(lengths.begin(), lengths.end());

        int num_clusters = lengths.size();
        int index_percent = static_cast<int>(num_clusters * percent);
        return lengths[index_percent];
    }

    void prepare_ivf_id_code(
            uint8_t* data_points,
            int* label,
            int* cluster_length,
            int n_cluster,
            int m) {
        uint64_t all_index = 0;
        int max_list_size = 0; 
        int min_list_size = 100 * 1000 * 1000;

        for (int i = 0; i < n_cluster; i++) {
            size_t list_size = invlists->list_size(i);

            InvertedLists::ScopedCodes scodes(invlists, i);
            const uint8_t* codes = scodes.get();

            InvertedLists::ScopedIds sids(invlists, i);
            ...
            for (size_t j = 0; j < list_size; j++) {
                label[all_index] = (int)ids[j];

                std::memcpy(
                        &data_points[all_index * m],
                        &codes[j * m],
                        m * sizeof(uint8_t));
                all_index++;
            }
...
struct InvertedListScanner {
    ...
    virtual void get_sim_table(float* sim_table_, int length) {
        printf("error, should not be here\n");
        return;
    };

    virtual void get_dis0(float& dis0_){
        printf("error, should not be here\n");  
        return;
    }
