...
__API_SYMBOL__ dpu_error_t dpu_check_wavegen_mux_status_for_dpu(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t *expected, int dpu_id2)
{
	dpu_error_t status;
	...
	if (!timeout) {
		...
		printf("Timeout waiting for result to be correct,dpu_id2 = %d,expected[0] = %d,expected[1] = %d,expected[2] = %d,expected[3] = %d,expected[4] = %d,expected[5] = %d,expected[6] = %d,expected[7] = %d\n",
		       dpu_id2, expected[0], expected[1], expected[2],
		       expected[3], expected[4], expected[5], expected[6],
		       expected[7]);
		printf("result_array[0] = %d,result_array[1] = %d,result_array[2] = %d,result_array[3] = %d,result_array[4] = %d,result_array[5] = %d,result_array[6] = %d,result_array[7] = %d\n",
		       result_array[0], result_array[1], result_array[2],
		       result_array[3], result_array[4], result_array[5],
		       result_array[6], result_array[7]);
...
__API_SYMBOL__ dpu_error_t fifo_dpu_check_wavegen_mux_status_for_dpu(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t *expected,
	uint8_t ci_mask)
{
	dpu_error_t status;
	uint8_t dpu_dma_ctrl;
	...
	if (!timeout) {
		...
		printf("Timeout waiting for result to be correct,expected[0] = %d,expected[1] = %d,expected[2] = %d,expected[3] = %d,expected[4] = %d,expected[5] = %d,expected[6] = %d,expected[7] = %d\n",
					      ...
					      dpu_ci_bitfield_t ci_mux_pos)
{
	...
	int dpu_id2 =
		(rank->rank_handler_allocator_id) * 64 + ci_id * 8 + dpu_id;
...
static dpu_error_t fifo_host_handle_access_for_dpu(struct dpu_rank_t *rank,
						   uint8_t dpu_id,
						   dpu_ci_bitfield_t ci_mux_pos)
{
	dpu_error_t status;
	...
	uint8_t nr_cis =
		rank->description->hw.topology.nr_of_control_interfaces;
	uint8_t ci_mask = ALL_CIS;
	uint8_t expected[DPU_MAX_NR_CIS];
	dpu_slice_id_t slice_id;

	FF(ufi_select_dpu_even_disabled(rank, &ci_mux_pos, dpu_id));
	FF(ufi_set_mram_mux(rank, ci_mux_pos, ci_mux_pos));
	...
	FF(fifo_dpu_check_wavegen_mux_status_for_dpu(rank, dpu_id, expected,
						     ci_mux_pos));

	// int c_id;
	// for (c_id = 0; c_id < 8; c_id++) {
	// 	if ((ci_mux_pos >> c_id) & 1) {
	// 		break;
	// 	}
	// }
	// int d_id = rank->rank_handler_allocator_id * 64 + c_id * 8 + dpu_id;
	// if (is_dpu_running(d_id) == true) {
	// 	exit(1);
	// }
	// int friend_id = d_id % 2 == 0 ? d_id + 1 : d_id - 1;
	// if (is_dpu_running(friend_id) == true) {
	// 	exit(1);
	// }
	...
	// 	for (j = 0; j < 8; j++) {
	// 		expected[j] = WAVEGEN_MUX_DPU_EXPECTED;
	// 	}
	// 	FF(ufi_select_dpu_even_disabled(rank, &ci_mask, i));
	// 	FF(dpu_check_wavegen_mux_status_for_dpu(rank, i, expected,
	// 						dpu_id2));
	// }
...
static dpu_error_t
release_fifo_host_handle_access_for_dpu(struct dpu_rank_t *rank, uint8_t dpu_id,
					dpu_ci_bitfield_t ci_mux_pos)
{
	dpu_error_t status;
	...
	FF(ufi_select_dpu_even_disabled(rank, &ci_mux_pos, dpu_id));
	FF(ufi_set_mram_mux(rank, ci_mux_pos, 0x0));

	for (slice_id = 0; slice_id < nr_cis; ++slice_id)
		expected[slice_id] = WAVEGEN_MUX_DPU_EXPECTED;
	...
	// 	}
	// 	FF(ufi_select_dpu_even_disabled(rank, &ci_mask, i));
	// 	FF(dpu_check_wavegen_mux_status_for_dpu(rank, i, expected, 0));
	// }
 ...
 * Do care about pairs of dpus: switch mux according to mask for both lines.
 */
__API_SYMBOL__ dpu_error_t fifo_dpu_switch_mux_for_dpu_line(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t mask)
{
	dpu_error_t status = DPU_OK;
	dpu_member_id_t dpu_pair_base_id = (dpu_member_id_t)(dpu_id & ~1);
	uint8_t nr_cis =
	...
	if (switch_base_line != switch_friend_line) {
		printf("error, switch_base_line = %d, switch_friend_line = %d\n",
		       switch_base_line, switch_friend_line);
		exit(1);
	}
	if (switch_base_line)
		FF(fifo_host_handle_access_for_dpu(rank, dpu_pair_base_id,
						   mask));
	if (switch_friend_line)
		FF(fifo_host_handle_access_for_dpu(rank, dpu_pair_base_id + 1,
						   mask));
 ...
 * Do care about pairs of dpus: switch mux according to mask for both lines.
 */
__API_SYMBOL__ dpu_error_t fifo_dpu_switch_single_mux_for_dpu_line(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t mask)
{
	dpu_error_t status = DPU_OK;
	uint8_t nr_cis =
	...
	for (each_slice = 0; each_slice < nr_cis; ++each_slice)
		mux_mram_mask |=
			dpu_get_host_mux_mram_state(rank, each_slice, dpu_id)
			<< each_slice;
	...
	for (each_slice = 0; each_slice < nr_cis; ++each_slice) {
		if (switch_base_line)
			dpu_set_host_mux_mram_state(rank, each_slice, dpu_id,
						    mask & (1 << each_slice));
	}
	...
	if (switch_base_line)
		FF(fifo_host_handle_access_for_dpu(rank, dpu_id, mask));
 ...
 * Do care about pairs of dpus: switch mux according to mask for both lines.
 */
__API_SYMBOL__ dpu_error_t release_fifo_dpu_switch_mux_for_dpu_line(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t mask)
{
	dpu_error_t status = DPU_OK;
	dpu_member_id_t dpu_pair_base_id = (dpu_member_id_t)(dpu_id & ~1);
	...
	for (each_slice = 0; each_slice < nr_cis; ++each_slice) {
		bit_this_slice = dpu_get_host_mux_mram_state(rank, each_slice,
							     dpu_pair_base_id);
		if (bit_this_slice == 0) {
			continue;
		}
		dpu_set_host_mux_mram_state(rank, each_slice, dpu_pair_base_id,
					    0);
		switch_base_line = true;
	}
	for (each_slice = 0; each_slice < nr_cis; ++each_slice) {
		bit_this_slice = dpu_get_host_mux_mram_state(
			rank, each_slice, dpu_pair_base_id + 1);
		if (bit_this_slice == 0) {
			continue;
		}
		dpu_set_host_mux_mram_state(rank, each_slice,
					    dpu_pair_base_id + 1, 0);
		switch_friend_line = true;
	}

	if (switch_base_line)
		FF(release_fifo_host_handle_access_for_dpu(
			rank, dpu_pair_base_id, mask));
	if (switch_friend_line)
		FF(release_fifo_host_handle_access_for_dpu(
			rank, dpu_pair_base_id + 1, mask));
 ...
 * Do care about pairs of dpus: switch mux according to mask for both lines.
 */
__API_SYMBOL__ dpu_error_t release_fifo_dpu_switch_single_mux_for_dpu_line(
	struct dpu_rank_t *rank, uint8_t dpu_id, uint8_t mask)
{
	dpu_error_t status = DPU_OK;
	uint8_t nr_cis =
	...
	for (each_slice = 0; each_slice < nr_cis; ++each_slice) {
		...
		dpu_set_host_mux_mram_state(rank, each_slice, dpu_id, 0);
		switch_base_line = true;
	}

	if (switch_base_line)
		FF(release_fifo_host_handle_access_for_dpu(rank, dpu_id, mask));
