 ...
 */
__API_SYMBOL__  dpu_error_t host_get_access_for_transfer_matrix(
...
__API_SYMBOL__  dpu_error_t fifo_host_get_access_for_transfer_matrix(
	struct dpu_rank_t *rank,
	void **ptr)
{
	...
	for (each_dpu = 0; each_dpu < nr_dpus_per_ci; each_dpu += 2) {
		...
		for (each_slice = 0; each_slice < nr_cis; ++each_slice) {
			...
			uint8_t get_mux_first_dpu =
				(ptr[idx_first_dpu] != NULL);
			uint8_t get_mux_second_dpu =
				(ptr[idx_second_dpu] != NULL);
		...
		if (mask1 || mask2) {
			...
			assert(!mask1 || !mask2 || (mask1 == mask2));
			mask = mask1 | mask2;
			// printf("threadid = %d, fifo_host_get_access_for_transfer_matrix: chip_dpu_id = %d, mask1 = %d, mask2 = %d, mask = %d\n", pthread_self(), each_dpu, mask1, mask2, mask);
			FF(fifo_dpu_switch_mux_for_dpu_line(rank, each_dpu, mask));
		}
...
__API_SYMBOL__  dpu_error_t fifo_host_release_access_for_transfer_matrix(
	struct dpu_rank_t *rank,
	void **ptr)
{
	dpu_error_t status = DPU_OK;
	...
	for (each_dpu = 0; each_dpu < nr_dpus_per_ci; each_dpu += 2) {
		...
		if (mask1 || mask2) {
			assert(!mask1 || !mask2 || (mask1 == mask2));
			mask = mask1 | mask2;
			// printf("thread_id = %d, fifo_host_release_access_for_transfer_matrix: chip_dpu_id = %d, mask1 = %d, mask2 = %d, mask = %d\n", pthread_self(), each_dpu, mask1, mask2, mask);
			FF(release_fifo_dpu_switch_mux_for_dpu_line(rank, each_dpu, mask));
